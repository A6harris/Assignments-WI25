...
class MonteCarloControl:
    ...
    def __init__(self, env: RaceTrack, gamma: float = 1.0, epsilon: float = 0.1, Q0: float = 0.0, max_episode_size : int = 1000):
        ...
        self.C = defaultdict(lambda: np.full(self.env.n_actions, 0))
    ...
    def create_target_greedy_policy(self):
        ...
        # Your code here
        for state in self.Q:
            action_probs = np.zeros(self.env.n_actions)
            best_action = np.argmax(self.Q[state])
            action_probs[best_action] = 1.0

            self.target_policy[state] = action_probs
    ...
    def create_behavior_egreedy_policy(self):
        ...
        for state in self.target_policy:
            action_probs = np.zeros(self.env.n_actions)
            greedy_action = np.argmax(self.target_policy[state])
            
            for a in range(self.env.n_actions):
                action_probs[a] = self.epsilon / self.env.n_actions
            
            action_probs[greedy_action] += (1 - self.epsilon)
            
            self.behavior_policy[state] = action_probs
    ...
    def egreedy_selection(self, state):
        ...
        actions = np.arange(self.env.n_actions)
        action_choice = np.random.choice(actions, p=action_probs)
    ...
    def generate_egreedy_episode(self):
        ...
        self.env.reset()
        episode = []
        current_state = tuple(self.env.get_state())
        for _ in range(self.max_episode_size):
            action = self.egreedy_selection(current_state)
    ...
    def generate_greedy_episode(self):
        ...
        current_state = tuple(self.env.get_state())   
        for _ in range(self.max_episode_size):
            action = int(np.argmax(self.target_policy[current_state]))
    ...
    def update_offpolicy(self, episode):
        ...
        for state, action, reward in reversed(episode):
            ...
            if self.C[state][action] > 0:  # Only update Q if C is positive
    ...
    def update_onpolicy(self, episode):
        ...
        for state, action, reward in reversed(episode):
            ...
            if sa_pair not in state_action_pairs:
                state_action_pairs.add(sa_pair)
